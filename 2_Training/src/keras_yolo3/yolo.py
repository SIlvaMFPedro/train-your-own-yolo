...
# -----------------------------
class YOLO(object):
    _defaults = {"model_path": "model_data/yolo.h5", "anchors_path": "model_data/yolo_anchors.txt",
                 "classes_path": "model_data/coco_classes.txt", "score": 0.3, "iou": 0.45,
    ...
    def generate(self):
        ...
        try:
            ...
            self.yolo_model = (
                tiny_yolo_body(Input(shape=(None, None, 3)), num_anchors // 2, num_classes) if is_tiny_version
                else yolo_body(Input(shape=(None, None, 3)), num_anchors // 3, num_classes)
            )
            self.yolo_model.load_weights(self.model_path)  # make sure model, anchors and classes match
        else:
            assert self.yolo_model.layers[-1].output_shape[-1] == num_anchors \
                    / len(self.yolo_model.output) * (num_classes + 5), "Mismatch between model, anchor and class sizes"
        end = timer()
    ...
    def detect_image(self, image, show_stats=True):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            # Image was expanded to model_image_size: make sure it did not pick up any box outside of the original
            # image (run into this bug when lowering confidence threshold to 0.01)
            ...
            for idx in range(thickness):
                draw.rectangle([left+idx, top+idx, right-idx, bottom-idx], outline=self.colors[c])
...
def detect_video(yolo, video_path, output_path=""):
    ...
    if not vid.isOpened():
        raise IOError("Could not open webcam or video file")
    ...
    if output_path != "":
        ...
        isOutput = False
    if isOutput:
        print("Processing {} with frame size {} at {:.1f} FPS".format(os.path.basename(video_path),
                                                                      video_size,
                                                                      video_fps))
