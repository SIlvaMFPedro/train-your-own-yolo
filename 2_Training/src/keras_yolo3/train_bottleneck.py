...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    # Create last layer model of the last layers from the YOLO model
...
def bottleneck_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, bottlenecks):
    """
        Bottleneck data generator function for fit_generator
        :param annotation_lines:
        :param batch_size:
        :param input_shape:
        :param anchors:
...
def main():
    ...
    model, bottleneck_model, last_layer_model = create_model(input_shape, anchors, num_classes, freeze_body=2,
                                                             weights_path="model_data/yolo_weights.h5")
    ...
    if True:
        ...
        if not os.path.isfile("bottlenecks.npz"):
            print("[INFO] Calculating bottlenecks...")
            batch_size = 8
            bottlenecks = bottleneck_model.predict_generator(data_generator_wrapper(lines, batch_size, input_shape,
                                                                                    anchors, num_classes,
        ...
        last_layer_model.fit_generator(bottleneck_generator(lines[:num_train], batch_size, input_shape,
                                                            anchors, num_classes, bottlenecks_train),
                                       steps_per_epoch=max(1, num_train // batch_size),
                                       validation_data=bottleneck_generator(lines[num_train:], batch_size, input_shape,
                                                                            anchors, num_classes, bottlenecks_val),
    ...
    if True:
        ...
        # Recompile the model to apply a change
