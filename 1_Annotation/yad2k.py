...
parser.add_argument('-flcl', '--fully_convolutional',
                    help='Model is fully convolutional so set input shape to (None, None, 3). ' 
                         'WARNING: This experimental option does not work properly for YOLO_v2.', action='store_true')


# -----------------------------
#   FUNCTIONS
# -----------------------------
def unique_config_sections(config_false):
    """
...
def main(args):
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith('convolutional'):
            ...
            if len(layers) > 1:
                print('[INFO] Concatenating route layers:', layers)
